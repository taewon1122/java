...
- 조회(무비차트)
    ...
    - 영화는 오늘을 기준으로 오늘 이후의 일정들을 예매한 내역을 기준으로 조회한다.
    - 영화는 예매 내역이 많은 순으로 조회한다.
- 조회(상영시간)
	- 사용자는 영화, 극장, 날짜를 선택해서 상영시간을 확인할 수 있다.
    - 사용자는 현재 상영중인 영화를 선택할 수 있다.
    - 사용자는 현재 영화가 상영중인 극장을 선택할 수 있다
    - 사용자는 현재 영화가 상용중인 날짜를 선택할 수 있다.
    - 사용자는 영화, 극장, 날짜를 선택하면 영화관 정보(극장 이름, 전체좌석수)와 상영시간 별 현재 에매 가능 좌석수를 조회할 수 있다.
    - 상세 정보는 감독, 배우, 장르, 기본정보, 개봉일, 트레일러, 스틸컷이 있다.
    - 장르를 등로된 장르를 보여주며, 장르는 여러가지일 수 있다.
	- 기본정보에는 관람 나이정보, 런닝 시간(분), 제작국가가 있다.
    - 제작 국가는 여러 국가가 될 수 있다.
    - 감독은 여러 감독이 올 수 있다.
    - 배우는 주연, 조연 구분 없이 등록된 배우를 영화에 등록된 배우순으로 조죄한다.
- 검색
...
- 예매
	- 예매는 회원만 가능하다.
    - 상영시간을 선택하면 인원을 선택하고, 좌석을 선택해서 에매를 할 수 있다.
    - 인원은 성인, 청소년으로 구분한다.
    - 좌석은 선택된 인원수에 맞춰서 에매 가능한 좌석을 선택한다.
    - 예매가 완료되면 예매 번호가 할당되고, 에매 번호를 통해 에매 좌석과 예매 인원 정보를 소개할 수 있다.
    - 예에시 할인은 없다.
...
- 극장 관리
	...
	- 극장은 지역, 이름, 상영관 정보, 주소를 입력하고, 총 관수를 입력하고 총 좌석수는 등록된 상영관에 맞게 계산이 된다.
	- 지역은 등록된 지역 중 하나를 선택한다.
	- 상영관 정보는 극장등록과 함께 등록하거나 또는 극장 등록 후 등록할 수 있다.
- 상영관 관리
